...
static void egg_tray_icon_get_property(GObject *object, guint prop_id,
...
GType egg_tray_icon_get_type(void) {
  ...
  if (our_type == 0) {
    static const GTypeInfo our_info = {sizeof(EggTrayIconClass),
                                       (GBaseInitFunc)NULL,
...
static void egg_tray_icon_class_init(EggTrayIconClass *klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_enum("orientation", _("Orientation"),
                        _("The orientation of the tray."), GTK_TYPE_ORIENTATION,
...
static void egg_tray_icon_get_orientation_property(EggTrayIcon *icon) {
  ...
  type = None;
  result =
      XGetWindowProperty(xdisplay, icon->manager_window, icon->orientation_atom,
                         0, G_MAXLONG, FALSE, XA_CARDINAL, &type, &format,
                         &nitems, &bytes_after, &(prop.prop_ch));
...
static void egg_tray_icon_send_manager_message(EggTrayIcon *icon, long message,
                                               Window window, long data1,
...
static void egg_tray_icon_update_manager_window(EggTrayIcon *icon,
  ...
  if (icon->manager_window != None)
    XSelectInput(xdisplay, icon->manager_window,
  ...
  if (icon->manager_window != None) {
    ...
    gdkwin = gdk_window_lookup_for_display(
        gtk_widget_get_display(GTK_WIDGET(icon)), icon->manager_window);
...
guint egg_tray_icon_send_message(EggTrayIcon *icon, gint timeout,
                                 const gchar *message, gint len) {
  guint stamp;
  ...
  while (len > 0) {
    ...
    XSendEvent(xdisplay, icon->manager_window, False, StructureNotifyMask,
               (XEvent *)&ev);
